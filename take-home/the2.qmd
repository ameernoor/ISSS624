...
distance <- sp::spDists(busstop_sp, longlat = FALSE)

hex_id_names <- busstop_sp$hexagon_id

colnames(distance) <- paste0(hex_id_names)
rownames(distance) <- paste0(hex_id_names)
...
flowline %>%
  ...
  tm_lines(col = 'coral',
           ...
           alpha = 0.2)+
  tm_layout(main.title = 'Weekday Morning Peak Flowline for 3rd Quantile', main.title.position = 'center')
```
...
flowline %>%
  ...
  tm_lines(col = 'coral',
           lwd = "trips",
           scale = c(0.1, 1, 2, 3, 4, 5, 6, 7))+
  tm_layout(main.title = 'Weekday Morning Peak Flowline for 99th Percentile', main.title.position = 'center')
```

## Filter by Distance
```{r}
summary(flowline$dist)
```
## Use GIF

# Assembling Push & Pull Factor
