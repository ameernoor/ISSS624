...
```{r}
# Keep one row of the duplicates in the original dataset
biz <- busstop[!duplicated(biz) | duplicated(biz, fromLast = TRUE), ]
...
n_distinct(entertn$POI_NAME)
n_distinct(entertn$POI_ST_NUM)
n_distinct(entertn$POI_ST_NAM)
n_distinct(entertn$POI_ID)
```
```{r}
duplicated_rows <- entertn[duplicated(entertn$POI_ID) | duplicated(entertn$POI_ID, fromLast = TRUE), ]
duplicated_rows
```

```{r}
# create a unique identifier
entertn <- mutate(entertn, POI_ID = paste(POI_NAME, POI_ST_NUM, POI_ST_NAM, sep = ' '))
...
## Derive Counts in Supporting Data
