...
```{r}
pacman::p_load(tmap, sf, sp, DT, performance,reshape2, ggupbr, units, tidyverse)
```
...
Continuing from **Chapter 15: Processing and Visualising Flow Data**, this exercise utilizes two key datasets:
...
Additionally, an attribute data file named *pop.csv* will be provided.
...
Spatial interaction involves computing a distance matrix, which tabulates distances between location pairs. For instance, the Euclidean distance between MESZ01 and RVSZ05 is 3926.0025, and between MESZ01 and SRSZ01 is 3939.1079. The distance from a location to itself (main diagonal) is always 0.

Learn how to compute a distance matrix in this section using the URA Master Plan 2019 Planning Subzone boundary (*mpsz.rds*) saved as an rds file. Import it into the R environment using the provided code chunk.
