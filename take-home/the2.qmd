...
```{r}
#| code-fold: false
pacman::p_load(tmap, sf, tidyverse, sfdep, knitr, Hmisc, mapview, DT, sp, stplanr, reshape2, skimr, performance, plotly, h3jsr, units, httr, corrplot, gifski)
```
...
tmap_animation(trips_spread, "../images/trips_spread.gif", loop = TRUE, delay = 1000,
               outer.margins = NA, restart.delay=1500)
```
![](../images/trips_spread.gif)
for the first 8 decile, the trips slightly change but still more dense on central.
on the extra category of >5000, north-downtown, west-downtown, north-east, and short trip becomes more visible

# Assemble Push & Pull Factors
...
push_poi_datasets <- c("busstop", "station", "trainexit", "hdb_sf")
...
hdb_capacity <- st_join(hdb_capacity, hexagons_with_bus_stops, by = NULL, join = st_within) %>%
  group_by(hexagon_id) %>%
  summarise(est_pop = sum(capacity)) %>%
  st_drop_geometry()
...
factors_holder <- left_join(factors_holder, hdb_capacity, by = "hexagon_id") %>%
  ...
  mutate_at(vars(contains("est_pop")), list(~coalesce(., 0.99)))  # Fill missing values with 0.99 
